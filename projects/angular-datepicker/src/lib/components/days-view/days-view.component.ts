...
import { CurrentCultureService, GlobalizationService, CldrService } from '@code-art/angular-globalize';
import {
    ...
    private calculate() {
        if (!this._calculated) {
            ...
            this._endDate = createDate(this.year, this.month,
                this.cldrService.getCalendar(this.locale || this.cultureService.currentCulture).getDaysInMonth(this.year, this.month));

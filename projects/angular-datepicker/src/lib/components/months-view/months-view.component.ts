...
@Component({
    selector: 'cadp-months-view',
    styleUrls: ['../../styles/year-month-view.scss'],
    ...
    public isDisabled(month: number): boolean {
        ...
        if (this.minDate) {
            const end = createDate(this.year, month,
                this.cldrService.getCalendar(this.locale || this.cultureService.currentCulture).getDaysInMonth(this.year, month));
